...
//计算选中单品列表
const selecedCartList = computed(() => {
  return cartList.value.filter((item) => item.selected)
})
//计算选中总件数
const selecedCartListCount = computed(() => {
  return selecedCartList.value.reduce((total, item) => {
    return total + item.count
  }, 0)
})
//计算选中总价格
const selecedCartListPrice = computed(() => {
  return selecedCartList.value
    .reduce((total, item) => {
...
<template>
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrolltolower">
    ...
    <template v-if="memberStore.profile">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar">
        <text class="all" @tap="onselectedAll" :class="{ checked: isSelectedAll }">全选</text>
        <text class="text">合计:</text>
        <text class="amount">{{ selecedCartListPrice }}</text>
        <view class="button-grounp">
          <view
            @tap="gotoPatment"
            class="button payment-button"
            :class="{ disabled: selecedCartListCount === 0 }"
          >
            去结算({{ selecedCartListCount }})
          </view>
        </view>
      </view>
    </template>
